...
def iter(virul,inf):
    ...
    writemod('variable')
    os.system("./sir " + str(numdays) + " usa control variable > output.csv")
    data = pandas.read_csv('output.csv')
    lastnonempty = numdays
    while data[tci][lastnonempty] == 0 and data[tvi][lastnonempty] == 0:
        lastnonempty = lastnonempty-1
    return (float(data[tvi][lastnonempty])+0.000001)/(float(data[tci][lastnonempty])+0.000001)
    ...
    for v in virul:
        ...
        for b in infec:
            ...
            if r < -20:
                ...
                rarray.append(r)
    print(ctrl['infectivity'],ctrl['virulence'])
    for r in rarray:
        maxr = max(maxr,abs(r))
    matplotlib.pyplot.title(title + '\n colored based on log2(variable prevalence/control prevalence) capped at +-20 \n control: infectivity ' + str(ctrl['infectivity']) + ' and virulence ' + str(ctrl['virulence']))
    fig = matplotlib.pyplot.scatter(x,y,c=rarray,cmap='coolwarm',vmin=-maxr,vmax=maxr)
    matplotlib.pyplot.scatter([ctrl['infectivity']],[ctrl['virulence']],s=[50],marker="x",c='black')
